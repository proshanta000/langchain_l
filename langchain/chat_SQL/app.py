...
# LLM Integration (Groq is in a partner package)
from langchain_groq import ChatGroq

# COMMUNITY IMPORTS (Utilities and Integrations)
...
# Initialize variables to avoid errors if radio_opt[1] isn't selected
...
if radio_opt.index(selected_opt) == 1:
    ...
    mysql_host = st.sidebar.text_input("Provide MySQL Host (e.g., localhost, IP).")
...
# 1. Stop if connection details are incomplete
...
def configure_db(db_uri, mysql_host=None, mysql_user=None, mysql_password=None, mysql_db=None):
    if db_uri == LOCALDB:
        ...
        # Using read-only mode for safety
...
try:
    ...
    # UPGRADED LLM to 70B for enhanced reasoning and tool-use capability
    ...
    sql_agent_executor = create_sql_agent(
        ...
        max_iterations=15 # Increased iteration limit to prevent premature stopping on complex tool use.
    )
